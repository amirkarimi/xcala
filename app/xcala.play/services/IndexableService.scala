...
import xcala.play.models.{Indexable, IndexedItem}
...
trait IndexableService[A <: Indexable] extends DataService with DataCollectionService with DataRemoveService with DataSaveService[A] with DataDocumentHandler[A]
{
  ...
  lazy val indexedItemCollection = {
    ...
    collectionFuture.map(_.indexesManager.ensure(
      Index(
        Seq(
  ...
  private def saveItem(id: BSONObjectID, model: Indexable): Future[WriteResult] = {
    val existingItem = indexedItemCollection.flatMap(_.find(BSONDocument("itemId" -> id, "itemType" -> model.itemType)).one[IndexedItem])
